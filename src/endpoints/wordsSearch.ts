...
export default class WordsSearch {
  ...
  async fetch(search: string, language?: string, pagesize?: number, page?: number) {
    try {
      const params = serializeParams({ "key":this.apiKey, "format":this.returnFormat, "action":"words-search", search, language, pagesize, page })
      const response = await this.axios.get(params);
      return response.data;
    } catch (error) {
      console.error("Error performing words search:", error);
